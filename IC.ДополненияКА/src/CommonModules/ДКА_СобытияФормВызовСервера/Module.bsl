...
Функция ДанныеРасширенногоПросмотра(Номенклатура) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		НаборТоварныхОграничений.Добавить(Новый Структура("ЗапасыСклад, min, max", Выборка.ЗапасыСклад, Выборка.min, Выборка.max));
	КонецЦикла;
